    ...
    $.fn.extend({
        smartpaginator : function(options) {
            ...
            return this.each(function() {
                ...
                var inputPage = $('<input/>').attr('type', 'text')
                    .keyup(function(e) {
                        var reg = new RegExp(/^[0-9]\d*$/);
                        var value = inputPage.val();
                        if(reg.test(value) && (value > 0 && value <= totalpages)) {
                ...
                function showLabels(pageIndex) {
                    container.find('span').remove();
                    var totalPage = Math.ceil(settings.totalrecords / settings.recordsperpage);
                    if (totalPage == 0) totalPage = 1;
                    container.append($('<span/>').text('共'))
                        .append($('<span/>').append($('<b/>').text(totalPage)))
                        .append($('<span/>').text('页'));
                }
