package com.xjt.core.shiro.session;
...
import com.xjt.core.utils.StringUtils;
import com.xjt.core.shiro.CustomShiroSessionDAO;
import com.xjt.manager.login.bo.UserOnlineBo;
/**
 * 
 * 开发公司：SOJSON在线工具 <p>
...
public class CustomSessionManager {
	...
	public List<SimplePrincipalCollection> getSimplePrincipalCollectionByUserId(Long...userIds){
		...
		for (Session session : sessions) {
			...
			if(null != obj && obj instanceof SimplePrincipalCollection){
				...
				if(null != obj && obj instanceof Home){
					Home user = (Home)obj;
					//比较用户ID，符合即加入集合
	...
	private UserOnlineBo getSessionBo(Session session){
		...
		if(obj instanceof SimplePrincipalCollection){
			...
			if(null != obj && obj instanceof Home){
				//存储session + user 综合信息
				UserOnlineBo userBo = new UserOnlineBo((Home)obj);
				//最后一次和系统交互的时间
